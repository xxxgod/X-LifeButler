...
public class SysMainController extends BaseController
{
    ...
    public String main(ModelMap mmap)
    {
        ...
        //收入支出曲线图
        List<XFinancial> income = records.stream().filter(record ->
                        record.getRecordDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate().getMonth() == currentMonth.getMonth() &&
                                record.getRecordDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate().getYear() == currentMonth.getYear()
                                && record.getType().equals("收入")
                ).collect(Collectors.toList());
        List<XFinancial> pay = records.stream().filter(record ->
                record.getRecordDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate().getMonth() == currentMonth.getMonth() &&
                        record.getRecordDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate().getYear() == currentMonth.getYear()
                        && record.getType().equals("支出")
        ).collect(Collectors.toList());
